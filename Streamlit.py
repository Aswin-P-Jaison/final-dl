...
task = st.radio("select the task you want", ("Sentiment Classification", "Tumor Detection"))
...
if task == "Sentiment Classification":
    ...
    if new_review_text.strip():
        st.subheader("Please select the model for Sentiment Classification")
        ...
        if model_option == "Perceptron":
            with open(r'perceptron_model.pkl', 'rb') as file:
                model = pickle.load(file)
        elif model_option == "Backpropagation":
            with open(r'imdb_backprop.pkl', 'rb') as file:
                model = pickle.load(file)
        elif model_option == "DNN":
            model = load_model(r'dnn_model.keras')
        elif model_option == "RNN":
            model = load_model(r'RNn_model.keras')
        elif model_option == "LSTM":
            model = load_model(r'lstm_model.keras')

        if st.button("Classify Sentiment"):
            result = sentiment_classification(new_review_text, model)
            st.subheader("Sentiment Classification Final Result")
            st.write(f"**{result}**")

elif task == "Tumor Detection":
    st.subheader("Tumor Detection")
    uploaded_file = st.file_uploader("Please upload a tumor image...", type=["jpg", "jpeg", "png"])

    if uploaded_file is not None:
        # Load the tumor detection model
        model = load_model(r'cnn_model.keras')
        ...
        if st.button("Detect Tumor"):
            ...
            st.subheader("Tumor Detection Final Result")
            st.write(f"**{result}**")
